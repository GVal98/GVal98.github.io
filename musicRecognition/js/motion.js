// Инструмент: запись акселерометра.
//
// Слух у приложения уже есть — оно само слышит, что заиграла музыка. Второй
// выключатель, ногой, писать было не из чего, пока неизвестно, чем одна поза
// отличается от другой в цифрах: какая ось её замечает, насколько разрыв между
// позами больше собственной дрожи и сколько времени занимает переход. Эта
// страница сняла ровно это, и её сводка показала главное — что порога по
// значению не существует вовсе (почему, см. README, «Включение ногой»).
//
// Выключатель после этого написан и лежит в js/pose.js, а страница осталась:
// её записи — в measures/, по ним гоняется test/pose.test.mjs, и если гейт
// начнёт врать в зале, чинить его придётся снова по записи, а не по памяти.
//
