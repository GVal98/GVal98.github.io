...
const DEFAULTS = {
  ...
  // Упрощённая азбука: шесть букв без собственного звука уходят из неё. Пять
  // заменяются созвучными (Q → K, Y → I, J → G, Z → S, V → W), шестая, C,
  // читается по соседу, как в английском: SITI, но KOLDPLAI. Включена, потому
  // что на ощупь считают знаки, а не буквы: все шесть кодов четырёхзначные
...
function refreshMorseHint() {
  ...
  $('setMorseSimpleHint').textContent =
    ...
    `иначе K («Coldplay» → KOLDPLAI); CH — это SH, а CK — один K: «Black» → BLAK. ` +
