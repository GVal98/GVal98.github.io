...
const DEFAULTS = {
  ...
  // Упрощённая азбука: шесть букв без собственного звука уходят из неё, пара EA
  // читается как I, а сдвоенные схлопываются в одну. Пять букв заменяются
  // созвучными (Q → K, Y → I, J → G, Z → S, V → W), шестая, C, читается
  // по соседу, как в английском: SITI, но KOLDPLAI. Включена, потому что
  // на ощупь считают знаки, а не буквы:
...
function refreshMorseHint() {
  ...
  // азбукой: «KUEN» сам по себе выглядит опечаткой, а не заменой.
  ...
  $('setMorseSimpleHint').textContent =
    ...
    `Пара EA — это I («The Beatles» → BITLE), кроме EAR, EAD и EATH: «Pearl Jam» и «Death Grips» остаются собой. ` +
